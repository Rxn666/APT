...
class Trainer():
    ...
    def forward_one_batch(self, inputs, targets, prompt_attribute, is_train):
        ...
        # prompt_attribute = prompt_attribute.to(self.device, non_blocking=True)
        inputs = inputs.cuda()  # (batchsize, 2048)
        targets = targets.cuda()  # (batchsize, )
        prompt_attribute = prompt_attribute.cuda()

        if self.cfg.DBG:
            logger.info(f"shape of inputs: {inputs.shape}")
            logger.info(f"shape of targets: {targets.shape}")
            logger.info(f"shape of prompt_attribute: {prompt_attribute.shape}")

        # forward
        with torch.set_grad_enabled(is_train):
            outputs = self.model(inputs, prompt_attribute)  # (batchsize, num_cls)
            if self.cfg.DBG:
                logger.info(
    ...
    def get_input(self, data):
        if self.cfg.DATA.NAME == 'ucf101':
            ...
            labels = data["label"]
            prompt_attribute = data["prompt_attribute"]
            return inputs, labels, prompt_attribute

        if self.cfg.DATA.NAME == 'hmdb51':
            if not isinstance(data["image"], torch.Tensor):
                for k, v in data.items():
                    data[k] = torch.from_numpy(v)
        ...
        if self.cfg.DATA.NAME == 'k400-tiny':
            # X = torch.cat(data['inputs'], dim=0)
            # label = []
            # for i in range(len(data['data_samples'])):
            #     label.append(data['data_samples'][i].gt_labels.item)
            # label = torch.tensor(label)
            # return X.float(), label
            (inputs, labels, _, meta) = data
    ...
    def train_classifier(self, train_loader, val_loader, test_loader):
        ...
        for epoch in range(total_epoch):
            ...
            for idx, input_data in enumerate(train_loader):
                ...
                X, targets, prompt_attribute = self.get_input(input_data)

                # measure data loading time
                data_time.update(time.time() - end)

                train_loss, _ = self.forward_one_batch(X, targets, prompt_attribute, True)
            ...
            try:
                ...
                curr_acc_5 = self.evaluator.results[f"epoch_{epoch}"]["classification"][t_name]["top5"]
            except KeyError:
                return
            ...
            if patience >= self.cfg.SOLVER.PATIENCE:
                ...
                break
            if self.cfg.TEST.WANDB:
                wandb.log({"lr": lr, "loss": losses.avg, "best_epoch": best_epoch,
                           "top1": curr_acc, "top5": curr_acc_5
                           })
    ...
    def eval_classifier(self, data_loader, prefix, save=False):
        ...
        for idx, input_data in enumerate(data_loader):
            ...
            loss, outputs = self.forward_one_batch(X, targets, prompt_attribute, False)
            if loss == -1:
                return
